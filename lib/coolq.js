...
const parseMessage = (message) => {
    winston.debug(`Parse: ${util.inspect(message)}`)
    let images = [];
    let records = [];
    let videos = [];
    let at = [];
...
let last_at = -1;
                ...
                case 'at':
                    ...
                    if (tmp && tmp[1]) {
                        if (tmp[1] === 'all') {
                            ...
                            at_q = parseInt(tmp[1]);
                            if(at_q == last_at) atstr='';
                            else {
                                at.push(at_q);
                                atstr = `@${tmp[1]}`;                // 只給出 QQ 號，至於應該 @ 什麼內容，讓使用者處理吧
                            }
                            last_at = at_q;
                        }
                    } else {
        ...
        winston.debug(util.inspect(message))
        let tmp, tmp2;
        ...
        for (let i = 0; i < message.length; i++) {
    			...
    			switch (curr.type) {
    			  case 'text': 
                  if (i >= 1 && message[i-1].type == 'text') {
                    text += ' ';
                  }
                  text += curr.data.text; break;
