        ...
        client.on('message', async (ctx, next) => {
            if (this._enabled && ctx.message && ctx.chat) {
                ...
                const context = new Context({
                    ...
                    to: ctx.chat.id,
                    nick: (await my_get_name(ctx.message.from.id)) || this._getNick(ctx.message.from),
                    text: '',
                    isPrivate: (ctx.chat.id > 0),
                    extra: {
                        username: ctx.message.from.username,
                        unified_id: id_cvt(ctx.message.from.id),
                    },
                    handler: this,
                    _rawdata: ctx,
                });
                ...
                if (ctx.message.reply_to_message) {
                    ...
                    let replyTo = await my_get_name(reply.from.id) || this._getNick(reply.from);
                    ...
                    context.extra.reply = {
                        ...
                        to_id: reply.from.id,
                        unified_to_id: id_cvt(reply.from.id),
                    };
                    ...
                    let fwdFrom = await my_get_name(fwd.id) || this._getNick(fwd);
                    // 用户昵称或频道名称
